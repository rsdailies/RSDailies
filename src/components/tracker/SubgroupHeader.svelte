	...
	<td colspan={colspan} class="header_like_color">
		<div class="header_like_inner">
			...
			<div class="header_like_controls">
				{#if rightText}<span class="header_like_status countdown badge bg-secondary">{rightText}</span>{/if}
				{#if showControls}
					{#if onReset}<button type="button" class="btn btn-secondary btn-sm mini-reset-btn primitive-btn" onclick={reset} title="Reset group">♻</button>{/if}
					{#if id}<button type="button" class="btn btn-secondary btn-sm mini-collapse-btn primitive-btn" onclick={toggle}>{isCollapsed ? '▶' : '▼'}</button>{/if}
				{/if}
			</div>
		</div>
	</td>
