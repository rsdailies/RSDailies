...
} from '../domain/legacy-mode-content.ts';
import { GAMES, getSelectedGame, subscribeToGameChanges } from './game-context.ts';
...
function syncViewsButtonLabel(button: HTMLElement | null, mode: string, game: string) {
	if (!button) return;
	const activeGame = getActiveGame(game);
	const modeDef =
...
function renderViewsList(list: HTMLElement, onSelectView: (mode: string) => void, game: string) {
	list.innerHTML = '';
...
function upsertPrimaryNavLinks(documentRef: Document, onSelectMode: (mode: string) => void, game: string) {
