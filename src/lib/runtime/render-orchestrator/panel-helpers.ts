...
export function getGroupCountdown(
	groupName: string,
	{
		...
		formatDurationMsCore: (diff: number) => string;
		nextDailyBoundary: (date?: Date) => Date;
		nextWeeklyBoundary: (date?: Date) => Date;
		nextMonthlyBoundary: (date?: Date) => Date;
	}
) {
	const lower = String(groupName || '')
		.toLowerCase()
		.trim();
	if (lower === 'general' || lower.includes('daily')) return formatDurationMsCore(nextDailyBoundary(new Date()).getTime() - Date.now());
	if (lower.includes('weekly')) return formatDurationMsCore(nextWeeklyBoundary(new Date()).getTime() - Date.now());
	if (lower.includes('monthly')) return formatDurationMsCore(nextMonthlyBoundary(new Date()).getTime() - Date.now());
	return '';
}

function getLeadingText(row: Element | null) {
	if (!row) return '';
...
export function movePenguinsBlockToBottom() {
	...
	while (cursor < rows.length) {
		...
		const isPenguinChild =
			row.classList.contains('weekly-child-row') || /^Penguin\s+\d+$/i.test(text) || text === 'Polar Bear';
...
export function appendCustomEmptyPlaceholder(tbody: HTMLElement | null) {
