import { renderSectionPanelHeader } from '../widgets/section-panel-header.ts';

function renderColgroup(columns: string[]) {
	return columns.map((columnClass) => `<col class="${columnClass}">`).join('');
}

export function buildSectionPanelHtml(section: any) {
	const shell = section.shell || {};
	const columns = Array.isArray(shell.columns)
		? shell.columns
		: ['activity_col_name', 'activity_col_notes', 'activity_col_status'];
