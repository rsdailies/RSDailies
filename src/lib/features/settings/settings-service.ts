...
export function normalizeSettings(partial: Partial<Settings> = {}): Settings {
	...
	return { splitDailyTables: partial.splitDailyTables !== false, splitWeeklyTables: partial.splitWeeklyTables !== false, showCompletedTasks: partial.showCompletedTasks === true || String(partial.showCompletedTasks) === 'true', herbTicks, growthOffsetMinutes: deriveGrowthOffsetMinutes(herbTicks), browserNotif: !!partial.browserNotif, webhookUrl, webhookUserId: webhookUrl ? normalizeWebhookUserId(partial.webhookUserId) : '', webhookMessageTemplate: normalizeWebhookTemplate(partial.webhookMessageTemplate), overviewVisible: partial.overviewVisible !== false };
}
