import { HEADER_CONTROL_LABELS } from './header-constants.ts';
import { canRenderRestoreMenu } from './header-logic.ts';
import { closeRestoreMenu, openRestoreMenu } from './restore-menu-logic.ts';

export function createRestoreMenu(restoreOptions: any[] = [], onRestoreSelect: ((value: string) => void) | null = null) {
	...
	applyButton.addEventListener('click', (event) => {
		...
		if (!select.value || !onRestoreSelect) return;
	...
	document.addEventListener('click', (event) => {
		if (!wrapper.contains(event.target as Node)) closeRestoreMenu(menu, trigger);
	});
