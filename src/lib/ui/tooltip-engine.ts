...
export function showTooltip(text: string, anchorRect: DOMRect, { documentRef = document, windowRef = window } = {}) {
...
export function buildTooltipText(task: any) {
	if (!task) return '';

	const hoverNote =
		typeof task.hover?.note === 'string'
...
export function attachTooltip(targetElement: HTMLElement | null, task: any, { documentRef = document, windowRef = window } = {}) {
