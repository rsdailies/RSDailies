import { ACTIVE_PROFILE_KEY, GLOBAL_PROFILES_KEY, createProfilePrefix } from '../shared/storage/namespace.ts';
...
function profileStorageKey(profileName: string, key: string) {
	return `${createProfilePrefix(profileName)}${key}`;
}

function getProfiles(storage: Storage) {
...
function ensureActiveProfile(profiles: string[], storage: Storage) {
...
function migrateProfileStorage(profileName: string, storage: Storage) {
