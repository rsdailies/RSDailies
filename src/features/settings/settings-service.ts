...
import { type Settings, settingsDefaults } from './settings-defaults.ts';
...
export function normalizeSettings(partial: Partial<Settings> = {}): Settings {
	...
	return {
		...
		showCompletedTasks: partial.showCompletedTasks === true || String(partial.showCompletedTasks) === 'true',
		densityMode,
		herbTicks,
...
export function getSettings(): Settings {
	return normalizeSettings(StorageService.load(StorageKeyBuilder.settings(), settingsDefaults));
}
...
export function updateSetting<K extends keyof Settings>(key: K, value: Settings[K]) {
	return saveSettings({ ...getSettings(), [key]: value });
}
