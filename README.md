...
- `public/` ó static images and public assets
- `src/pages/` ó Astro routes
- `src/app/` ó shared Astro shell layout, app state, and global style layers
- `src/features/` ó Svelte feature modules for tracker, timers, settings, navigation, and modals
- `src/entities/` ó domain types plus static content adapters
- `src/shared/` ó storage, time, API helpers, shared UI, and utilities
- `src/content/` ó tracker page and section data
- `src/content/docs/` ó internal project documentation
- `tests/` ó project tests
- `tools/` ó validation and audit scripts

Current source layout is centered around:

- `src/app/` √¢‚Ç¨‚Äù shell layout, app state, and global style entrypoints
- `src/features/` √¢‚Ç¨‚Äù tracker, timers, navigation, settings, and modal workflows
- `src/entities/` √¢‚Ç¨‚Äù task and game domain models plus static content adapters
- `src/shared/` √¢‚Ç¨‚Äù storage, time, utilities, and shared UI primitives
- `src/content/docs/` √¢‚Ç¨‚Äù internal documentation that replaced the old standalone docs tree
